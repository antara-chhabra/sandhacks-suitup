...
# Paths (backend at repo root; extras in extras/)
BASE_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = BASE_DIR.parent
UPLOADS_DIR = BASE_DIR / "uploads"
SESSIONS_DIR = BASE_DIR / "sessions"
SPEECH_INTENT_DIR = PROJECT_ROOT / "extras" / "speech_intent"
VISUAL_PERCEPTION_DIR = BASE_DIR / "visual_perception"
INTERVIEWQS_DIR = PROJECT_ROOT / "extras" / "interviewqs_comppos"
# Fallback if running from suit-up-web/app/backend
if not VISUAL_PERCEPTION_DIR.exists():
    _alt = PROJECT_ROOT / "suit-up-web" / "visual_perception"
    if _alt.exists():
        VISUAL_PERCEPTION_DIR = _alt
if not SPEECH_INTENT_DIR.exists():
    _alt = PROJECT_ROOT / "speech_intent"
    if _alt.exists():
        SPEECH_INTENT_DIR = _alt
if not INTERVIEWQS_DIR.exists():
    _alt = PROJECT_ROOT / "interviewqs_comppos"
    if _alt.exists():
        INTERVIEWQS_DIR = _alt
